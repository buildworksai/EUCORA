...
class DeploymentIntent(TimeStampedModel, CorrelationIdModel):
    ...
    status = models.CharField(
        max_length=20, choices=Status.choices, default=Status.PENDING, help_text="Deployment status"
    )
    ...
    submitter = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="deployment_intents", help_text="User who submitted the deployment"
    )
...
class RingDeployment(TimeStampedModel):
    ...
    connector_type = models.CharField(
        max_length=20, help_text="Connector type (intune, jamf, sccm, landscape, ansible)"
    )
    connector_object_id = models.CharField(
        max_length=255, null=True, blank=True, help_text="Platform-specific object ID"
    )
