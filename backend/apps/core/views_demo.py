...
"""
                ...
                try:
                    ...
                    result = subprocess.run(
                        ['psql', '-h', db_host, '-p', str(db_port), '-U', db_user, '-d', db_name, 
                         '-v', 'ON_ERROR_STOP=0', '-f', tmp_sql_file],
                        capture_output=True,
                        text=True,
                        timeout=120,
                        env=env
        ...
        if force_seed or has_form_parameters or not (use_backup_only and backup_file and backup_file.exists()):
            logger.info(f'Seeding demo data using seeding function with parameters: assets={assets}, applications={applications}, deployments={deployments}, users={users}, events={events}...')
            ...
            return Response(
                {'status': 'error', 'error': 'Backup file not found and use_backup_only is True. Set force_seed=true to seed instead.'},
