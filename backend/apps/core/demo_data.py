...
from apps.connectors.models import Application, Asset
...
def demo_data_stats() -> dict:
    ...
    return {
        ...
        + User.objects.filter(username="demo").count(),
    }
...
def _seed_demo_users(count: int) -> None:
    ...
    for i in range(remaining):
        ...
        user = User.objects.create_user(
            username=username, email=f"{username}@demo.eucora.com", password=DEMO_USER_PASSWORD
        )
...
def _seed_assets(count: int, batch_size: int) -> int:
    ...
    for i in range(remaining):
        ...
        disk_enc = (
            random.choice([True, True, True, True, False]) if status == "Active" else random.choice([True, False])
        )
...
def _seed_deployments(count: int, demo_user: User, batch_size: int) -> dict:
    ...
    application_definitions = [
        ...
        },
        {
            ...
            "vendor": "Adobe",
            "base_risk": 55,
            "versions": ["23.001.20143", "23.002.20191", "23.003.20244"],
        },
        {
            "name": "Google Chrome",
            "vendor": "Google",
            "base_risk": 40,
            "versions": ["119.0.6045.105", "120.0.6099.109", "121.0.6167.85"],
        },
        {
            "name": "Slack",
            "vendor": "Slack Technologies",
            "base_risk": 35,
            "versions": ["4.35.121", "4.36.134", "4.37.141"],
        },
        {
            "name": "Zoom Client",
            "vendor": "Zoom Video",
            "base_risk": 50,
            "versions": ["5.16.0.24010", "5.17.0.24080", "5.17.5.24100"],
        },
        {
            "name": "Visual Studio Code",
            "vendor": "Microsoft",
            "base_risk": 30,
            "versions": ["1.84.2", "1.85.1", "1.86.0"],
        },
        {
            "name": "CrowdStrike Falcon",
            "vendor": "CrowdStrike",
            "base_risk": 70,
            "versions": ["7.10.16808", "7.11.16903", "7.12.17005"],
        },
        {
            "name": "Microsoft Defender",
            "vendor": "Microsoft",
            "base_risk": 65,
            "versions": ["4.18.23110.2", "4.18.24010.7", "4.18.24020.4"],
        },
            ...
            for _ in range(num_deployments_for_version):
                ...
                evidence_batch.append(
                    EvidencePack(
                        ...
                        is_validated=status
                        not in [DeploymentIntent.Status.PENDING, DeploymentIntent.Status.AWAITING_CAB],
                        is_demo=True,
                    )
                )
                ...
                if len(deployments_batch) >= batch_size:
                    evidence_packs_created += len(
                        EvidencePack.objects.bulk_create(evidence_batch, ignore_conflicts=True)
                    )
                    evidence_batch = []
                    ...
                    saved = {
                        d.correlation_id: d
                        for d in DeploymentIntent.objects.filter(
                    ...
                    if cab_batch:
                        ...
                        cab_approvals_created += len(
                            CABApproval.objects.bulk_create(cab_objects, ignore_conflicts=True)
                        )
                        cab_batch = []
                    ...
                    if ring_batch:
                        ...
                        for ring_entry in ring_batch:
                            ...
                            if deployment:
                                success_count = (
                                    ...
                                    if ring_entry["status"] == DeploymentIntent.Status.COMPLETED
                                    else random.randint(0, 50)
                                )
                                failure_count = (
                                    random.randint(0, 25)
                                    if ring_entry["status"]
                                    in [DeploymentIntent.Status.FAILED, DeploymentIntent.Status.ROLLED_BACK]
                        ...
                        ring_deployments_created += len(
                            RingDeployment.objects.bulk_create(ring_objects, ignore_conflicts=True)
                        )
                        ring_batch = []
...
def _generate_realistic_sbom(app_name: str, version: str) -> dict:
    ...
    return {
        ...
        "relationships": [{"type": "DEPENDS_ON", "from": app_name, "to": pkg["name"]} for pkg in selected_packages],
    }
...
def _generate_realistic_vulnerabilities(app_name: str, version: str) -> dict:
    ...
    cve_templates = {
        "critical": [
            ...
            {
                "id": "CVE-2024-5678",
                "title": "Critical Buffer Overflow in zlib",
                "package": "zlib",
                "version": "1.2.13",
            },
        ...
        "high": [
            ...
            {
                "id": "CVE-2024-3456",
                "title": "Information Disclosure in Python",
                "package": "python",
                "version": "3.11.5",
            },
            {
                "id": "CVE-2024-4567",
                "title": "Cross-Site Scripting in Electron",
                "package": "electron",
                "version": "25.3.1",
            },
        ],
        "medium": [
            {
                "id": "CVE-2024-6789",
                "title": "Denial of Service in HTTP Parser",
                "package": "nodejs",
                "version": "20.5.0",
            },
