...
class CircuitBreakerListener:
    ...
    def success(self, cb):
        ...
        if cb.state.name == "half-open":
            logger.info(f"Circuit breaker recovery successful: {self.name}", extra={"circuit_breaker": self.name})
...
def get_breaker(service_name: str) -> CircuitBreaker:
    ...
    if not breaker:
        raise ValueError(
            f"Unknown service: {service_name}. " f"Available: {', '.join(sorted(_BREAKER_REGISTRY.keys()))}"
        )
    return breaker
...
def check_breaker_status(service_name: str) -> bool:
    ...
    if breaker.state.name == "open":
        logger.warning(
            f"Circuit breaker OPEN for service: {service_name}", extra={"service": service_name, "state": "OPEN"}
        )
