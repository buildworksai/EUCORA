...
class TestDeploymentIntentModels:
    ...
    def test_deployment_intent_str_representation(self, db):
        ...
        intent = DeploymentIntent.objects.create(
            deployment_id="test-deploy", asset_id="asset-1", package_version="1.0.0"
        )
    ...
    def test_deployment_intent_get_absolute_url(self, db):
        ...
        intent = DeploymentIntent.objects.create(
            deployment_id="test-deploy-2", asset_id="asset-2", package_version="1.0.0"
        )
...
class TestCABWorkflowModels:
    ...
    def test_cab_approval_str_representation(self, db):
        ...
        approval = CABApproval.objects.create(
            deployment_intent_id="test-intent", approver=approver, status="pending", risk_score=30.0
        )
    ...
    def test_cab_approval_status_choices(self, db):
        ...
        for status in valid_statuses:
            approval = CABApproval.objects.create(
                deployment_intent_id=f"test-{status}", approver=approver, status=status, risk_score=30.0
            )
...
class TestPolicyEngineModels:
    ...
    def test_policy_str_representation(self, db):
        ...
        policy = Policy.objects.create(name="test-policy", description="Test", rules={})
    ...
    def test_policy_is_active_default(self, db):
        ...
        policy = Policy.objects.create(name="default-active-policy", rules={})
...
class TestEvidenceStoreModels:
    ...
    def test_evidence_package_str_representation(self, db):
        ...
        evidence = EvidencePackage.objects.create(deployment_intent_id="test-intent", risk_score=30.0, evidence_data={})
    ...
    def test_evidence_package_timestamps(self, db):
        ...
        evidence = EvidencePackage.objects.create(
            deployment_intent_id="test-intent-2", risk_score=40.0, evidence_data={}
        )
...
class TestConnectorModels:
    ...
    def test_connector_str_representation(self, db):
        ...
        connector = Connector.objects.create(name="Test Intune", connector_type="intune", config={}, is_active=True)
    ...
    def test_connector_active_status(self, db):
        ...
        active = Connector.objects.create(name="Active", connector_type="intune", config={}, is_active=True)

        inactive = Connector.objects.create(name="Inactive", connector_type="jamf", config={}, is_active=False)
...
class TestAIAgentModels:
    ...
    def test_ai_conversation_relationships(self, db):
        ...
        from apps.ai_agents.models import AIAgentType, AIConversation

        user, created = User.objects.get_or_create(username="user1", defaults={"email": "u@test.com"})
        ...
        conv = AIConversation.objects.create(user=user, agent_type=AIAgentType.AMANI_ASSISTANT, title="Test")
    ...
    def test_ai_message_content_stored(self, db):
        ...
        from apps.ai_agents.models import AIAgentType, AIConversation, AIMessage

        user, created = User.objects.get_or_create(username="user2", defaults={"email": "u2@test.com"})
        ...
        msg = AIMessage.objects.create(conversation=conv, role="user", content="Hello AI", token_count=2)
...
class TestModelQuerysetMethods:
    ...
    def test_deployment_intent_filter_by_state(self, db):
        ...
        DeploymentIntent.objects.create(
            deployment_id="deploy1", asset_id="asset1", package_version="1.0.0", state="not_started"
        )

        DeploymentIntent.objects.create(
            deployment_id="deploy2", asset_id="asset2", package_version="1.0.0", state="completed"
        )
    ...
    def test_cab_approval_filter_by_status(self, db):
        ...
        CABApproval.objects.create(deployment_intent_id="test1", approver=approver, status="pending")

        CABApproval.objects.create(deployment_intent_id="test2", approver=approver, status="approved")
    ...
    def test_event_store_filter_by_type(self, db):
        ...
        DeploymentEvent.objects.create(event_type="deployment_started", deployment_intent_id="test", actor="system")

        DeploymentEvent.objects.create(event_type="deployment_completed", deployment_intent_id="test", actor="system")
