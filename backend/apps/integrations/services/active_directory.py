...
class ActiveDirectoryService(IntegrationService):
    ...
    def test_connection(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not all([server_url, username, password]):
            return {
                'status': 'failed',
                'message': 'Missing required configuration (server_url, username, password)'
            }
    ...
    def sync(self, system: ExternalSystem) -> Dict[str, Any]:
        ...
        return {
            'fetched': users_created + users_updated + groups_created + groups_updated + computers_created + computers_updated,
        ...
        try:
            ...
            for entry in conn.entries:
                ...
                for attr in attributes:
                    ...
                    if value:
                        user_dict[attr] = str(value[0]) if hasattr(value, '__iter__') and not isinstance(value, str) else str(value)
        ...
        try:
            ...
            for entry in conn.entries:
                ...
                for attr in attributes:
                    value = getattr(entry, attr, None)
                    if value:
                        group_dict[attr] = str(value[0]) if hasattr(value, '__iter__') and not isinstance(value, str) else str(value)
    ...
    def _fetch_computers(self, system: ExternalSystem) -> List[Dict[str, Any]]:
        ...
        attributes = ['cn', 'name', 'distinguishedName', 'operatingSystem', 'operatingSystemVersion', 'lastLogon', 'userAccountControl']
        ...
        try:
            ...
            for entry in conn.entries:
                ...
                for attr in attributes:
                    value = getattr(entry, attr, None)
                    if value:
                        computer_dict[attr] = str(value[0]) if hasattr(value, '__iter__') and not isinstance(value, str) else str(value)
