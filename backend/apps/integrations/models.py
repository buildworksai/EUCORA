...
class ExternalSystem(TimeStampedModel, CorrelationIdModel):
    ...
    api_url = models.URLField(
        help_text='Base URL for the external system API'
    )
    ...
    credentials = models.JSONField(
        default=dict,
        help_text='Vault path reference for encrypted credentials (never store plaintext)'
    )
    sync_interval_minutes = models.IntegerField(
        default=60,
        help_text='How often to sync data from this system (in minutes)'
    )
    ...
    metadata = models.JSONField(
        default=dict,
        help_text='System-specific configuration (field mappings, filters, etc.)'
    )
    is_demo = models.BooleanField(
        default=False,
        help_text='Whether this is demo/test integration data'
    )
...
class IntegrationSyncLog(TimeStampedModel, CorrelationIdModel):
    ...
    records_fetched = models.IntegerField(
        default=0,
        help_text='Number of records fetched from external system'
    )
    records_created = models.IntegerField(
        default=0,
        help_text='Number of new records created in local database'
    )
    records_updated = models.IntegerField(
        default=0,
        help_text='Number of existing records updated in local database'
    )
    ...
    duration_seconds = models.FloatField(
        null=True,
        blank=True,
        help_text='Sync duration in seconds'
    )
