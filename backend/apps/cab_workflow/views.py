...
from apps.core.throttles import BurstRateThrottle, StrictRateThrottle
...
def list_pending_approvals(request):
    ...
    deployments = apply_demo_filter(
        DeploymentIntent.objects.filter(status=DeploymentIntent.Status.AWAITING_CAB, requires_cab_approval=True),
        request,
    )
    ...
    for deployment in deployments:
        ...
        if approval.decision == decision_filter:
            ...
            evidence_pack_correlation_id = (
                deployment.evidence_pack_id if deployment.evidence_pack_id else deployment.correlation_id
            )
            ...
            approvals.append(
                {
                    ...
                    "evidence_pack_correlation_id": str(
                        evidence_pack_correlation_id
                    ),  # Add evidence pack correlation_id
