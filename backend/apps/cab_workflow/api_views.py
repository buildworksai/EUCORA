...
def submit_cab_approval(request):
    ...
    try:
        ...
        return Response(
            {'error': f'Evidence package not found: {evidence_package_id}'},
...
def get_cab_request(request, cab_request_id):
    ...
    try:
        ...
        return Response(
            {'error': 'CAB request not found'},
            status=status.HTTP_404_NOT_FOUND
        )
    
    # Check authorization (requester or CAB member or staff)
    if (cab_request.submitted_by != request.user and 
        not request.user.is_staff and 
        not request.user.groups.filter(name__in=['cab_member', 'security_reviewer']).exists()):
        return Response(
...
def list_my_cab_requests(request):
    ...
    """
    queryset = CABApprovalRequest.objects.filter(
        submitted_by=request.user
...
def approve_cab_request(request, cab_request_id):
    ...
    if not (request.user.is_staff or 
            request.user.groups.filter(name='cab_member').exists()):
        return Response(
            {'error': 'Not authorized (CAB member access required)'},
            status=status.HTTP_403_FORBIDDEN
        )
...
def reject_cab_request(request, cab_request_id):
    ...
    try:
        ...
        return Response(
            {
                **response_serializer.data,
                'message': 'CAB request rejected successfully'
            },
            status=status.HTTP_200_OK
        )
...
def list_my_exceptions(request):
    ...
    """
    queryset = CABException.objects.filter(
        requested_by=request.user
...
def approve_exception(request, exception_id):
    ...
    if not (request.user.is_staff or 
            request.user.groups.filter(name='security_reviewer').exists()):
        return Response(
            {'error': 'Not authorized (Security Reviewer access required)'},
            status=status.HTTP_403_FORBIDDEN
        )
...
def cleanup_expired_exceptions(request):
    ...
    try:
        ...
        return Response({
            'message': f'Cleaned up {count} expired exceptions',
            'count': count
        })
    except Exception as e:
