...
class CABWorkflowTestSetup(TestCase):
    ...
    def setUpTestData(cls):
        ...
        # Create evidence package first (to get its ID)
        evidence_id = uuid.uuid4()
        cls.evidence = EvidencePackage.objects.create(
            id=evidence_id,
            deployment_intent_id="",  # Will be updated after deployment creation
        ...
        # Create deployment intent with evidence_pack_id
        ...
        # Update evidence with deployment ID
...
class TestCABSubmission(CABWorkflowTestSetup):
    ...
    def test_submit_missing_evidence_package(self):
        ...
        nonexistent_uuid = str(uuid4())  # Use valid UUID format
        with self.assertRaises(ValueError) as ctx:
            CABWorkflowService.submit_for_approval(
                evidence_package_id=nonexistent_uuid,
                deployment_intent_id=str(self.deployment.id),
                risk_score=Decimal("40"),
                submitted_by=self.requester,
            )
    ...
    def test_submit_creates_auto_approved_decision(self):
        ...
        decision = CABApprovalDecision.objects.filter(cab_request_id=str(cab_req.id)).first()
...
class TestCABApproval(CABWorkflowTestSetup):
    ...
    def test_approve_creates_decision_record(self):
        ...
        decisions = CABApprovalDecision.objects.filter(cab_request_id=str(self.cab_req.id))
