...
class CABAPITestSetup(TestCase):
    ...
    def setUpTestData(cls):
        ...
        # Create users
        cls.requester, created = User.objects.get_or_create(
            username="requester", defaults={"email": "requester@example.com"}
        )

        if created:

            cls.requester.set_password("testpass123")

            cls.requester.save()

        cls.cab_member, created = User.objects.get_or_create(
            username="cab_member", defaults={"email": "cab@example.com"}
        )

        if created:

            cls.cab_member.set_password("testpass123")

            cls.cab_member.save()

        cls.security_reviewer, created = User.objects.get_or_create(
            username="security_reviewer", defaults={"email": "security@example.com"}
        )

        if created:

            cls.security_reviewer.set_password("testpass123")

            cls.security_reviewer.save()

        cls.unauthorized_user, created = User.objects.get_or_create(
            username="unauthorized", defaults={"email": "unauthorized@example.com"}
        )

        if created:

            cls.unauthorized_user.set_password("testpass123")
...
class TestCABSubmitEndpoint(CABAPITestSetup):
    ...
    def test_submit_requires_authentication(self):
        ...
        response = self.client.post(
            "/api/v1/cab/submit/",
            {"evidence_package_id": str(self.evidence.id), "risk_score": "45.0", "notes": "Test submission"},
        )
    ...
    def test_submit_low_risk_auto_approved(self):
        ...
        response = self.client.post(
            "/api/v1/cab/submit/",
            {"evidence_package_id": str(self.evidence.id), "risk_score": "40.0", "notes": "Low risk deployment"},
        )
    ...
    def test_submit_medium_risk_pending_review(self):
        ...
        response = self.client.post(
            "/api/v1/cab/submit/",
            {"evidence_package_id": str(self.evidence.id), "risk_score": "60.0", "notes": "Medium risk deployment"},
        )
    ...
    def test_submit_high_risk_exception_required(self):
        ...
        response = self.client.post(
            "/api/v1/cab/submit/",
            {"evidence_package_id": str(self.evidence.id), "risk_score": "80.0", "notes": "High risk deployment"},
        )
    ...
    def test_submit_invalid_evidence_package(self):
        ...
        response = self.client.post("/api/v1/cab/submit/", {"evidence_package_id": "nonexistent", "risk_score": "40.0"})
    ...
    def test_submit_invalid_risk_score_negative(self):
        ...
        response = self.client.post(
            "/api/v1/cab/submit/", {"evidence_package_id": str(self.evidence.id), "risk_score": "-10.0"}
        )
    ...
    def test_submit_invalid_risk_score_too_high(self):
        ...
        response = self.client.post(
            "/api/v1/cab/submit/", {"evidence_package_id": str(self.evidence.id), "risk_score": "101.0"}
        )
    ...
    def test_submit_returns_correlation_id(self):
        ...
        response = self.client.post(
            "/api/v1/cab/submit/", {"evidence_package_id": str(self.evidence.id), "risk_score": "45.0"}
        )
...
class TestCABRetrievalEndpoints(CABAPITestSetup):
    ...
    def setUp(self):
        ...
        response = self.client.post(
            "/api/v1/cab/submit/",
            {"evidence_package_id": str(self.evidence.id), "risk_score": "60.0", "notes": "Test request"},
        )
...
class TestCABApprovalEndpoints(CABAPITestSetup):
    ...
    def setUp(self):
        ...
        self.client.force_authenticate(user=self.requester)
        response = self.client.post(
            "/api/v1/cab/submit/", {"evidence_package_id": str(self.evidence.id), "risk_score": "60.0"}
        )
        self.cab_request_id = response.data["id"]
        self.client.force_authenticate(user=None)
    ...
    def test_approve_requires_cab_member(self):
        ...
        response = self.client.post(f"/api/v1/cab/{self.cab_request_id}/approve/", {"rationale": "Approved"})
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_approve_by_cab_member(self):
        ...
        response = self.client.post(
            f"/api/v1/cab/{self.cab_request_id}/approve/", {"rationale": "Risk acceptable, approved for deployment"}
        )
    ...
    def test_approve_with_conditions(self):
        ...
        response = self.client.post(
            ...
            {"rationale": "Approved with conditions", "conditions": conditions},
            format="json",
        )
    ...
    def test_reject_by_cab_member(self):
        ...
        response = self.client.post(
            f"/api/v1/cab/{self.cab_request_id}/reject/", {"rationale": "Risk assessment incomplete"}
        )
    ...
    def test_cannot_approve_twice(self):
        ...
        # First approval
        self.client.post(f"/api/v1/cab/{self.cab_request_id}/approve/", {"rationale": "Approved"})

        # Second approval should fail
        response = self.client.post(f"/api/v1/cab/{self.cab_request_id}/approve/", {"rationale": "Approved again"})
...
class TestCABExceptionEndpoints(CABAPITestSetup):
    ...
    def test_approve_exception_requires_security_reviewer(self):
        ...
        self.client.force_authenticate(user=self.unauthorized_user)
        response = self.client.post(f"/api/v1/cab/exceptions/{exception_id}/approve/", {"rationale": "Approved"})
    ...
    def test_approve_exception_by_security_reviewer(self):
        ...
        response = self.client.post(
            f"/api/v1/cab/exceptions/{exception_id}/approve/", {"rationale": "Controls sufficient"}
        )
