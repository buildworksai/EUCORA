...
class CABApprovalRequest(models.Model):
    ...
    risk_score = models.DecimalField(
        max_digits=5,
        decimal_places=2,
        help_text="Risk score from evidence package (0-100)"
    )
...
class CABApprovalDecision(models.Model):
    ...
    cab_request_id = models.CharField(
        max_length=255,
        help_text="ID of CABApprovalRequest being decided"
    )
    correlation_id = models.CharField(
        max_length=255,
        help_text="Audit trail identifier"
    )
    ...
    conditions = models.JSONField(
        default=dict,
        help_text="Conditions if decision is conditional"
    )
    
    class Meta:
        db_table = 'cab_workflow_cabrovaldecsion'
        ordering = ['-decided_at']
