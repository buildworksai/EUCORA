...
class CABApprovalRequest(models.Model):
    ...
    risk_score = models.DecimalField(max_digits=5, decimal_places=2, help_text="Risk score from evidence package")
    ...
    approved_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="cab_approvals"
    )
...
class CABApprovalDecision(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4)
    approval_request = models.OneToOneField(CABApprovalRequest, on_delete=models.CASCADE, related_name="decision")
    correlation_id = models.CharField(max_length=255, help_text="Links to deployment intent audit trail")
    ...
    # Evidence
    decision_maker = models.ForeignKey(User, on_delete=models.PROTECT, related_name="cab_decisions")
    decision_rationale = models.TextField(help_text="Why this decision was made")
    risk_score_at_decision = models.DecimalField(
        max_digits=5, decimal_places=2, help_text="Risk score at time of decision"
    )
