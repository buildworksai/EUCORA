...
from .models import CABApprovalDecision, CABApprovalRequest, CABException
...
class CABApprovalSubmitSerializer(serializers.Serializer):
    ...
    evidence_package_id = serializers.CharField(max_length=255, help_text="ID of evidence package to submit")
    risk_score = serializers.DecimalField(
        max_digits=5, decimal_places=2, min_value=Decimal("0"), max_value=Decimal("100"), help_text="Risk score 0-100"
    )
    notes = serializers.CharField(
        max_length=1000, required=False, allow_blank=True, help_text="Optional submission notes"
    )
...
class CABApprovalActionSerializer(serializers.Serializer):
    ...
    rationale = serializers.CharField(max_length=2000, help_text="Approval or rejection rationale")
    conditions = serializers.JSONField(
        required=False, allow_null=True, help_text="Optional conditions if conditionally approved (JSON object)"
    )
...
class CABExceptionCreateSerializer(serializers.Serializer):
    ...
    deployment_intent_id = serializers.CharField(max_length=255, help_text="Deployment intent ID for exception")
    reason = serializers.CharField(max_length=2000, help_text="Why exception is needed")
    risk_justification = serializers.CharField(max_length=2000, help_text="Why risk is acceptable despite threshold")
    compensating_controls = serializers.ListField(
        child=serializers.CharField(max_length=500), help_text="List of compensating controls"
    )
    expiry_days = serializers.IntegerField(
        default=30, min_value=1, max_value=90, help_text="Days until exception expires (1-90)"
    )
