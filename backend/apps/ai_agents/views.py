...
def delete_provider_by_type(request, provider_type):
    ...
    try:
        ...
        if count == 0:
            return Response(
                {'error': f'No providers found for type: {provider_type}'},
                status=status.HTTP_404_NOT_FOUND
            )
        ...
        return Response({
            'success': True,
            'message': f'Deleted {count} provider(s) of type {provider_type}'
        })
    except Exception as e:
        ...
        'pending_approvals': [
            {
                ...
                'initiated_by': {
                    ...
                    'name': f"{task.initiated_by.first_name} {task.initiated_by.last_name}".strip() or task.initiated_by.username,
                } if task.initiated_by else None,
            }
            for task in tasks
        ],
        ...
        return Response({
            'task': {
                ...
                'approved_by': {
                    ...
                    'name': f"{task.approved_by.first_name} {task.approved_by.last_name}".strip() or task.approved_by.username,
...
def approve_task(request, task_id):
    ...
    try:
        ...
        if not (request.user.is_staff or request.user.is_superuser):
            return Response(
                {'error': 'You do not have permission to approve tasks'},
                status=status.HTTP_403_FORBIDDEN
            )
