...
class DeploymentFlowIntegrationTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.submitter = User.objects.create_user(username='submitter', password='test123')
        ...
        self.risk_model = RiskModel.objects.create(
            version='1.0',
            factors={'unsigned': 50, 'no_sbom': 30, 'no_scan': 40}
        )
...
class CABApprovalFlowIntegrationTests(APITestCase):
    ...
    def test_events_recorded_for_approval(self):
        ...
        initial_events = DeploymentEvent.objects.filter(
    ...
    def test_rejection_updates_deployment_status(self):
        ...
        response = self.client.post(url, {
            'comments': 'Rejected: insufficient testing'
        }, format='json')
...
class EvidencePackGenerationIntegrationTests(APITestCase):
    ...
    def setUp(self):
        self.client = APIClient()
        self.user = User.objects.create_user(username='publisher', password='test123')
    ...
    def test_evidence_immutability_enforced(self):
        ...
        if response.status_code in [status.HTTP_200_OK, status.HTTP_201_CREATED]:
            ...
            update_response = self.client.put(f'/api/v1/evidence/{pack_id}/', {
...
class ConnectorPublishingFlowIntegrationTests(APITestCase):
    ...
    def setUp(self):
        self.client = APIClient()
        self.publisher = User.objects.create_user(username='publisher', password='test123')
...
class AuditTrailIntegrityTests(APITestCase):
    ...
    def test_user_actions_tracked(self):
        ...
        submitter = User.objects.create_user(username='submitter2', password='test')
        ...
        if DeploymentIntent.Status.AWAITING_CAB == deployment.status:
            ...
            approver = User.objects.create_user(username='approver2', password='test')
...
class IdempotencyValidationTests(APITestCase):
    ...
    def setUp(self):
        self.client = APIClient()
        self.user = User.objects.create_user(username='idempotent-test', password='test123')
        self.client.force_authenticate(user=self.user)
    ...
    def test_repeated_deployment_creation_idempotent(self, mock_risk_score):
        ...
        # First creation
        response1 = self.client.post('/api/v1/deployments/', {
        ...
        response2 = self.client.post('/api/v1/deployments/', {
            'app_name': 'idempotent-app',
            'version': '1.0.0',
            'target_ring': 'CANARY',
            'evidence_pack_id': str(evidence_id)
    ...
    def test_repeated_publication_idempotent(self, mock_publish):
        ...
        response1 = self.client.post('/api/v1/connectors/publish/', {
            'deployment_id': str(deployment.correlation_id),
            'target_plane': 'INTUNE'
        }, format='json')
        ...
        self.assertIn(response2.status_code, [
            status.HTTP_200_OK,
            status.HTTP_409_CONFLICT  # Conflict if already published
        ])
