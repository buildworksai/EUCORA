...
def entra_id_login(request):
    ...
    try:
        ...
        logger.error(
            f"User profile fetch failed: {e}", extra={"correlation_id": getattr(request, "correlation_id", "N/A")}
        )
        return Response({"error": "Failed to fetch user profile"}, status=status.HTTP_401_UNAUTHORIZED)
