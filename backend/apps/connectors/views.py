        ...
        assets.append(
            {
                ...
                "last_seen": (
                    asset.last_checkin.isoformat() if asset.last_checkin else asset.created_at.isoformat()
                ),  # Frontend expects 'last_seen'
...
def list_connectors(request):
    ...
    """
    connector_types = list(services.PowerShellConnectorService.CONNECTOR_SCRIPTS.keys())
    ...
    for conn_type in connector_types:
        connectors.append(
            {
                "type": conn_type,
                "name": conn_type.capitalize(),
                "status": "available",  # Could be enhanced to check actual health
            }
        )
...
def health_check(request, connector_type):
    ...
    if connector_type not in services.PowerShellConnectorService.CONNECTOR_SCRIPTS:
        return Response({"error": f"Invalid connector type: {connector_type}"}, status=status.HTTP_400_BAD_REQUEST)
