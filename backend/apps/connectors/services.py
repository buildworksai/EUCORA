...
class PowerShellConnectorService:
    ...
    def health_check(self, connector_type: str) -> Dict[str, Any]:
        ...
        if connector_type not in self.CONNECTOR_SCRIPTS:
            return {"status": "unhealthy", "message": f"Unknown connector type: {connector_type}", "details": {}}
        ...
        try:
            ...
            return {"status": "unhealthy", "message": "Health check timed out (timeout 30s)", "details": {}}
        except Exception as e:
            logger.error(f"PowerShell health check error: {e}")
            return {"status": "unhealthy", "message": f"Health check error: {str(e)}", "details": {}}
    ...
    def deploy(self, connector_type: str, deployment_params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            ps_params = [
                ...
                "Deploy",
                "-DeploymentIntentId",
                deployment_params["deployment_intent_id"],
                "-ArtifactPath",
                deployment_params["artifact_path"],
                "-TargetRing",
                deployment_params["target_ring"],
                "-AppName",
                deployment_params["app_name"],
                "-Version",
                deployment_params["version"],
            ]
