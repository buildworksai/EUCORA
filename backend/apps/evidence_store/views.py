...
def upload_evidence_pack(request):
    ...
    if not all([app_name, version, artifact_file]):
        return Response(
            {'error': 'app_name, version, and artifact file are required'},
    ...
    try:
        ...
        artifact_path, artifact_hash = storage.upload_artifact(
            artifact_file,
            object_name,
            content_type=artifact_file.content_type or 'application/octet-stream'
        )
    except Exception as e:
        logger.error(
            f'Failed to upload artifact to MinIO: {e}',
