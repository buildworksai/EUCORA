...
class EvidenceGenerationService:
    ...
    def _compute_risk_score(evidence_data: dict) -> tuple:
        ...
        if total_weight > 0:
            ...
            risk_score = Decimal('100.0') if raw_score > Decimal('100') else (Decimal('0.0') if raw_score < Decimal('0') else raw_score)
        else:
    ...
    def verify_package_immutability(package: EvidencePackage) -> dict:
        ...
        return {
            ...
            'message': 'Evidence package hash verification passed' if is_valid else 'Hash mismatch - evidence may have been modified',
        }
