...
class EvidencePackage(models.Model):
    ...
    deployment_intent_id = models.CharField(
        max_length=255,
        help_text="Correlation ID linking to deployment intent"
    )
    ...
    evidence_data = models.JSONField(
        default=dict,
        help_text="Complete evidence pack (artifacts, tests, scans, etc.)"
    )
    ...
    risk_factors = models.JSONField(
        default=dict,
        help_text="Breakdown of all contributing risk factors"
    )
    risk_model_version = models.CharField(
        max_length=50,
        default="1.0",
        help_text="Version of risk scoring model used"
    )
    
    # Completeness
    is_complete = models.BooleanField(
        default=False,
        help_text="All required fields populated"
    )
    completeness_check = models.JSONField(
        default=dict,
        help_text="Field-by-field completeness status"
    )
    ...
    created_by = models.CharField(
        max_length=255,
        default="system",
        help_text="User or system that created this package"
    )
    
    # Content Hash (immutability proof)
    content_hash = models.CharField(
        max_length=64,
        help_text="SHA-256 hash of evidence_data for immutability verification"
    )
...
class RiskFactor(models.Model):
    ...
    model_version = models.CharField(
        max_length=50,
        default="1.0",
        help_text="Risk model version this factor belongs to"
    )
    ...
    description = models.TextField(
        help_text="What this factor measures"
    )
    weight = models.DecimalField(
        max_digits=5,
        decimal_places=4,
        help_text="Percentage weight in overall score (0-1)"
    )
    rubric = models.JSONField(
        default=dict,
        help_text="Scoring rubric (e.g., {'>90%': 0, '<90%': 20})"
    )
...
class RiskScoreBreakdown(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid4)
    evidence_package = models.OneToOneField(
        EvidencePackage,
    ...
    correlation_id = models.CharField(
        max_length=255,
        help_text="Link to deployment intent"
    )
    
    factors_evaluated = models.JSONField(
        default=dict,
        help_text="All factors with their scores and weights"
    )
