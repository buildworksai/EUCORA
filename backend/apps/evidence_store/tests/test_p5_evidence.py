...
class EvidenceGenerationServiceTests(TestCase):
    ...
    def test_generate_evidence_package_complete(self):
        ...
        evidence_data = {
            ...
            'test_results': {
                'coverage_percent': 95,
                'manual_test_status': 'completed'
            },
            'scan_results': {'critical': [], 'high': []},
            'deployment_plan': {
                'affected_components': ['app1', 'app2', 'app3']
            },
            'rollback_plan': {
                'strategy': 'version_pinning',
                'steps': ['step1', 'step2']
            }
        }
...
class RiskScoreComputationTests(TestCase):
    ...
    def test_medium_risk_score(self):
        ...
        evidence_data = {
            ...
            'test_results': {
                'coverage_percent': 85,
                'manual_test_status': 'in_progress'
            },
...
class EvidenceRetrievalTests(TestCase):
    ...
    def test_verify_package_immutability(self):
        ...
        self.assertEqual(result['package_id'], str(package.id))
