...
export function AssetActionRunner({ assetId }: { assetId: string }) {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-full">
            ...
            {/* Console Output / Log */}
            <div className="md:col-span-1 rounded-xl bg-black/80 border border-white/10 font-mono text-xs p-4 flex flex-col">
                <div className="mb-2 text-muted-foreground border-b border-white/10 pb-2 flex justify-between">
                    <span>Console Output</span>
                    <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                </div>
                <ScrollArea className="flex-1">
                    <div className="space-y-2 text-green-400/80">
                        <div>&gt; Connecting to agent {assetId.substring(0, 8)}... OK</div>
                        ...
                        {Object.keys(history).map(actionId => {
                            ...
                            return (
                                <div key={actionId} className="space-y-1 mt-4">
                                    ...
                                    <div className="text-muted-foreground ml-2">... Stop-Service -Name "Spooler"</div>
                                    <div className="text-muted-foreground ml-2">... Start-Service -Name "Spooler"</div>
